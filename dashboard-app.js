...
function renderQuestionAnalysis(finishedStudents) {
    ...
    // แสดงส่วนวิเคราะห์เฉพาะเมื่อมีคนส่งข้อสอบแล้วเท่านั้น
    if (!finishedStudents || finishedStudents.length === 0) {
    ...
    let wrongCount = Array(8).fill(0);

    finishedStudents.forEach(s => {
        if (s.objectiveAnswers) {
            s.objectiveAnswers.forEach((val, idx) => { 
                if (val === 1) correctCount[idx]++; 
                else if (val === 0) wrongCount[idx]++;
            });
        }
    });

    // คำนวณสถิติสำหรับข้อที่ตอบถูกและตอบผิด
    const correctStats = correctCount.map((count, i) => ({ q: i + 1, count: count }));
    const wrongStats = wrongCount.map((count, i) => ({ q: i + 1, count: count }));
    
    const sortedCorrect = [...correctStats].sort((a, b) => b.count - a.count);
    const sortedWrong = [...wrongStats].sort((a, b) => b.count - a.count);
    ...
    const renderItem = (item, type) => `
        <div onclick="showQuestionDetail(${item.q})" class="flex justify-between items-center py-2 border-b border-slate-50 hover:bg-rose-50/30 cursor-pointer transition-all px-1 rounded group">
            ...
            <span class="text-xs font-bold ${type === 'wrong' && item.count > 0 ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-600'} px-2 py-0.5 rounded">${item.count} ครั้ง</span>
        </div>
    `;

    correctEl.innerHTML = sortedCorrect.slice(0, 3).map(item => renderItem(item, 'correct')).join('');
    wrongEl.innerHTML = sortedWrong.slice(0, 3).map(item => renderItem(item, 'wrong')).join('');
}
...
function updateDetailView(students) {
    ...
    if (document.getElementById('detailNo')) document.getElementById('detailNo').textContent = s.no || '-';
    ...
    objGrid.innerHTML = Array(8).fill(0).map((_, i) => {
        const val = objAnswers[i];
        let displayText = 'ไม่ได้ทำ';
        ...
        return `
            <div class="flex flex-col items-center p-2 rounded-lg border ${bgClass}">
                <span class="text-[9px] font-bold text-slate-400 mb-1">${i+1}</span>
                <span class="text-sm font-black ${colorClass}">${displayText}</span>
            </div>
        `;
    }).join('');
...
function showQuestionDetail(qNum) {
    ...
    // ค้นหานักเรียนที่ตอบผิดในข้อนี้
    const finishedData = JSON.parse(localStorage.getItem('mattepExamRecords') || '[]');
    const wrongStudents = finishedData.filter(s => s.objectiveAnswers && s.objectiveAnswers[qNum - 1] === 0);

    let extraHtml = '';
    if (wrongStudents.length > 0) {
        extraHtml = `
            <div class="text-[11px] font-bold text-rose-500 mb-3 uppercase tracking-wider">รายชื่อนักเรียนที่ตอบผิด (${wrongStudents.length} ครั้ง)</div>
            <div class="max-h-48 overflow-y-auto pr-1 flex flex-col gap-2">
                ${wrongStudents.map(s => `
                    <div class="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100 shadow-sm">
                        <span class="text-sm font-bold text-slate-700">${s.name || 'ไม่ระบุชื่อ'}</span>
                        <span class="text-[10px] text-slate-500 font-medium bg-white px-2 py-1 rounded-lg border border-slate-100">เลขที่ ${s.studentNo} • ห้อง ${s.studentClass}</span>
                    </div>
                `).join('')}
            </div>
        `;
    } else {
        extraHtml = `<div class="text-center py-4 text-emerald-500 text-sm font-bold">ยอดเยี่ยม! ยังไม่มีใครตอบผิดในข้อนี้</div>`;
    }

    const extraContainer = document.getElementById('modalExtraContent');
