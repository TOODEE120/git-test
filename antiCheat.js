...
class AntiCheat {
  ...
  _onKeyDown(e) {
    ...
    if (e.key === 'F12' || (e.ctrlKey && e.shiftKey && ['I', 'J', 'C'].includes(key)) || (e.ctrlKey && key === 'U')) {
      e.preventDefault();
      this._addViolation('DevTools Attempt', 50, 'devtools', `Key: ${e.key}`);
      return;
    }
  ...
  _onVisibilityChange() {
    if (document.hidden) {
      ...
      this._addViolation('Tab Hidden', 50, 'tabSwitch', 'ออกจากแท็บข้อสอบ');
      return;
    }
    ...
    if (this.lastVisibilityHiddenAt) {
      ...
      if (awayMs > 5000) {
        this._addViolation('Away From Exam', 50, 'tabSwitch', `ออกจากหน้า ${Math.round(awayMs / 1000)} วินาที`);
      }
      this.lastVisibilityHiddenAt = null;
    }
  }
  ...
  _onBlur() {
    if (!window.__mattepSubmitting && !this._isRecent('blur', 2500)) {
      this._addViolation('Window Blur', 50, 'tabSwitch', 'สลับไปหน้าต่างอื่น');
    }
  }
  ...
  _monitorDevtoolsResize() {
    setInterval(() => {
      ...
      if ((widthGap > 180 || heightGap > 220) && !this._isRecent('devtools-resize', 6000)) {
        this._addViolation('Possible DevTools', 50, 'devtools', 'ขนาดหน้าต่างคล้ายเปิดเครื่องมือนักพัฒนา');
      }
    }, 4000);
  }
